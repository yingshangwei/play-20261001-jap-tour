---
status: Active
owner: jap-tour
last_verified: 2026-09-04
sources:
  - MAP.md
  - README.md
  - docs/requirements/index.md
  - docs/development/index.md
  - docs/exec_plan/index.md
  - docs/research/index.md
---

# 项目文档索引

## 文档入口

- [仓库文档地图](../MAP.md)
- [项目说明](../README.md)
- [行程固定事实与编辑约束](../AGENTS.md)

## 分区

| 分区 | 入口 | 覆盖内容 |
| --- | --- | --- |
| 需求 | [requirements/index.md](requirements/index.md) | 目标、范围、约束、验收标准 |
| 开发 | [development/index.md](development/index.md) | 架构、模型、代码边界、技术决策 |
| 执行计划 | [exec_plan/index.md](exec_plan/index.md) | 阶段任务、进度、验证和阻塞 |
| 研究 | [research/index.md](research/index.md) | 外部证据、调研和方案比较 |

## 当前工作入口

- [攻略平台需求](requirements/guide-platform/requirements.md)
- [Day 2–9 每日手账需求](requirements/day-journals/requirements.md)
- [酒店调研需求](requirements/hotels/requirements.md)
- [页面文案去 AI 味需求](requirements/copy-editing/requirements.md)
- [配置化架构设计](development/guide-platform-architecture.md)
- [配置化重构执行计划](exec_plan/guide-config-refactor.md)
